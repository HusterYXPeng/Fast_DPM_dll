...
void FastDPM::detect( Mat &img, float score_thresh /* = -FLOAT_INF */, bool show_hints /* = true */, bool show_img /* = true */, string save_img /* = */ )
{
	...
	// 1. Feature pyramid <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<	
	if( hints )
	{ printf("Calculating feature pyramid ...\t"); start_clock = prag_start; }
	/* method 1. */
	...
	if (show_img || !save_img.empty()){
		...
		if (show_img)
			...
			//imwrite("D:/2017   labview 中文版本/自己弄的/Source cpp Files/FastDPM-2015-Nov/FastDPM-2014-May/photo_src/第二步处理/result_person.jpg", img_color);  //自己加的   2017/11/30
		if (!save_img.empty())
